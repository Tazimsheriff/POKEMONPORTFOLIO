...
const MoveList = ({ region = 'kanto' }) => {
    ...
    return (
        <section id="moves" className="py-24 px-6 relative overflow-hidden">
            <div className={`absolute top-1/2 left-0 w-64 h-64 ${bgColor.replace('bg-', 'bg-')}/5 rounded-full blur-[100px] -translate-x-1/2 overflow-hidden`} />
            <div className={`absolute bottom-0 right-0 w-96 h-96 ${bgColor.replace('bg-', 'bg-')}/5 rounded-full blur-[120px] translate-x-1/4 overflow-hidden`} />
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="mb-16">
                    ...
                    <p className="text-gray-400 max-w-2xl font-sans text-lg">
                        A versatile set of moves refined over years of training across the {region} region.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {skills.map((s, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className={`group relative glass-morphism border-2 border-white/5 rounded-xl p-4 md:p-6 hover:${borderColor.replace('/30', '/100')} transition-all cursor-crosshair overflow-hidden`}
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-4">
                                    <div className={`p-3 rounded-lg bg-white/5 group-hover:${s.bg} group-hover:text-black transition-colors`}>
                                        <s.icon className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <h3 className={`font-pixel text-[10px] md:text-xs text-white uppercase group-hover:${region === 'hoenn' ? 'text-poke-blue' : 'text-poke-yellow'} transition-colors`}>
                                            {s.name}
                                        </h3>
                                        <span className={`text-[8px] font-pixel uppercase ${s.color}`}>TYPE / {s.type}</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <span className="font-pixel text-[8px] text-gray-500 uppercase block mb-1">POWER</span>
                                    <span className="font-pixel text-sm text-white">{s.power}</span>
                                </div>
                            </div>
                            ...
                            <div className="h-2 bg-black/50 border border-white/10 rounded-full overflow-hidden p-[1px]">
                                <motion.div
                                    ...
                                    className={`h-full rounded-full ${s.bg} shadow-[0_0_10px_rgba(255,255,255,0.2)]`}
                                />
                            </div>
                            <div className={`absolute bottom-0 left-0 w-full h-0.5 ${bgColor} scale-x-0 group-hover:scale-x-100 transition-transform origin-left`} />
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="mt-20 p-8 border-4 border-dashed border-white/5 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-8 glass-morphism">
                    <div className="flex items-center gap-6">
                        <div className={`w-20 h-20 ${bgColor} rounded-full flex items-center justify-center animate-bounce shadow-[0_0_20px_rgba(255,255,255,0.2)] border-2 border-white/20`}>
                            <Zap className="w-10 h-10 text-white" />
                        </div>
                        <div>
                            <h4 className={`font-pixel text-sm mb-2 ${region === 'hoenn' ? 'text-poke-blue' : 'text-poke-yellow'}`}>HIDDEN ABILITY</h4>
                            <p className="text-gray-400 text-sm font-sans uppercase tracking-widest leading-relaxed">Rapid Learning & Problem Solving</p>
                        </div>
                    </div>
